...
class BuildTests(test_utils.GenericTestBase):
    """Test the build methods, including lazy-loading and CSS optimization."""

    def test_lazy_loading_added_to_images(self) -> None:
        """Test that process_html adds lazy-loading to all images."""
        html_content = '<html><body><img src="image1.jpg"><img src="image2.png"></body></html>'
        expected_output = '<html><body><img src="image1.jpg" loading="lazy"><img src="image2.png" loading="lazy"></body></html>'
        input_stream = io.StringIO(html_content)
        output_stream = io.StringIO()

        build.process_html(input_stream, output_stream)
        self.assertEqual(output_stream.getvalue(), expected_output)

    def test_font_display_swap_added_to_css(self) -> None:
        """Test that _minify_css adds font-display: swap to @font-face rules."""
        css_content = '''
        @font-face {
        ...
        expected_output = (
            ...
            'format("woff2");font-display:swap;}body{margin:0;padding:0;}'
        )
        source_stream = io.StringIO(css_content)
        target_stream = io.StringIO()

        with utils.open_file('test.css', 'w') as source_file:
            source_file.write(css_content)
        build._minify_css('test.css', 'minified_test.css')
        with utils.open_file('minified_test.css', 'r') as minified_file:
            self.assertEqual(minified_file.read(), expected_output)

    def test_flaky_test_fix(self) -> None:
        """Test that the flaky test has been fixed and runs deterministically."""
        flaky_test_function = 'test_creates_multiple_stats_models_from_multiple_users'

        try:
            # Simulating test behavior
            build_test_result = build.run_flaky_test(flaky_test_function)
            self.assertTrue(build_test_result)
        except AssertionError as e:
            self.fail(f"Flaky test {flaky_test_function} failed: {e}")
