...
# Add support for lazy loading of images by default in HTML
def process_html(
    source_file_stream: TextIO, target_file_stream: TextIO
) -> None:
    """Remove whitespaces and add hashes to filepaths in the HTML file stream
    object. Adds lazy-loading attribute for images.
    ...
    content = re.sub(r'(<img[^>]+)', r'\1 loading="lazy"', content)
...
def _minify_css(source_path: str, target_path: str) -> None:
    ...
    with utils.open_file(source_path, 'r') as source_file:
        ...
        # Add fallback fonts for visual stability (e.g., font-display: swap)
        css_content = re.sub(
            r'@font-face {[^}]*}',
            lambda m: m.group(0) + ' font-display: swap;',
            css_content
        )
        with utils.open_file(target_path, 'w') as target_file:
            target_file.write(rcssmin.cssmin(css_content))
